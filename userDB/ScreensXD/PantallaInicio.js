...
export default function PantallaInicio({ navigation }) {
  ...
  useEffect(() => { cargarTutorias(); }, []);

  const cargarTutorias = () => {
    obtenerTutorias((data) => { setTutorias(data); });
  };

  const categorias = ['Todas', 'Matemáticas', 'Programación', 'Ciencias', 'Idiomas'];

  const tutoriasEjemplo = tutorias.length ? tutorias : [
    { id:1, materia:"Cálculo Diferencial", tutor:"Victor Manuel", rating:4.9, reseñas:45, precio:"$80-$100 MX/hr", avatar:"VM", categoria:"Matemáticas", disponibilidad:"Disponible hoy" },
    { id:2, materia:"Programación Python", tutor:"José María", rating:5.0, reseñas:32, precio:"$80-$100 MX/hr", avatar:"JM", categoria:"Programación", disponibilidad:"Mañana 3 PM" }
  ];
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={styles.searchContainer}>
            ...
            <TextInput style={styles.searchInput} placeholder="Buscar materias o tutores..." placeholderTextColor="#9CA3AF" />
          </View>
        </View>
        ...
        <View style={styles.categoriesContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoriesScroll}>
            {categorias.map((categoria) => (
              <TouchableOpacity key={categoria} style={[ styles.categoryButton, categoriaSeleccionada === categoria && styles.categoryButtonSelected ]} onPress={() => setCategoriaSeleccionada(categoria)}>
                <Text style={[ styles.categoryText, categoriaSeleccionada === categoria && styles.categoryTextSelected ]}>{categoria}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.tutoriasSection}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity><Text style={styles.verTodasText}>Ver todas</Text></TouchableOpacity>
          </View>

          <View style={styles.tutoriasList}>
            {tutoriasEjemplo.map((tutoria) => (
              <TouchableOpacity key={tutoria.id} style={styles.tutoriaCard} onPress={() => navigation.navigate('PerfilTutor')}>
                <View style={styles.tutoriaHeader}>
                  <View style={styles.avatarContainer}><Text style={styles.avatarText}>{tutoria.avatar}</Text></View>
                ...
                <View style={styles.tutoriaFooter}>
                  <View style={styles.categoriaBadge}><Text style={styles.categoriaText}>{tutoria.categoria}</Text></View>
...
const styles = StyleSheet.create({
  ...
  scrollView:{flex:1},
  header:{backgroundColor:'#8B5CF6',paddingHorizontal:20,paddingTop:20,paddingBottom:24},
  ...
  notificationBadge:{position:'absolute',top:4,right:4,backgroundColor:'#EC4899',width:16,height:16,borderRadius:8,justifyContent:'center',alignItems:'center'},
  notificationText:{color:'white',fontSize:10,fontWeight:'bold'},
  searchContainer:{flexDirection:'row',alignItems:'center',backgroundColor:'white',borderRadius:12,paddingHorizontal:12},
  searchIcon:{marginRight:8},
  searchInput:{flex:1,paddingVertical:12,fontSize:16,color:'#1F2937'},
  categoriesContainer:{backgroundColor:'white',borderBottomWidth:1,borderBottomColor:'#E5E7EB',paddingVertical:16},
  categoriesScroll:{paddingHorizontal:20},
  categoryButton:{paddingHorizontal:16,paddingVertical:8,borderRadius:20,marginRight:8,backgroundColor:'#F3F4F6'},
  ...
  tutoriasList:{gap:12},
  tutoriaCard:{backgroundColor:'white',borderRadius:12,padding:16,borderWidth:1,borderColor:'#E5E7EB'},
  ...
  categoriaBadge:{backgroundColor:'#F3F4F6',paddingHorizontal:8,paddingVertical:4,borderRadius:6},
